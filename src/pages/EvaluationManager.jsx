...
import {
  ...
  Plus, Minus, Maximize2, Trash2, Layout, Award
} from 'lucide-react';
...
function EvaluationReviewModal({ submission, allBankQuestions, homeworks, curriculumData, bookTests, onClose, onSaveSuccess }) {
    ...
    async function loadTestData() {
      ...
      for (let i = 0; i < exactCount; i++) {
        ...
        generatedQuestions.push({
          ...
          questionText: existingQ.questionText || (exactCount === 1 ? (resolved?.questionText || resolved?.title) : `Soru ${i + 1}`),
  ...
  const { totalEarned, maxPossible, scorePercentage } = useMemo(() => {
    ...
    for (let i = 1; i <= qCount; i++) {
      const s = questionScores[i] ?? (answers[i - 1]?.score !== undefined ? Number(answers[i - 1].score) : (answers[i - 1]?.isCorrect === true ? 10 : 0));
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 99999, display: 'flex', flexDirection: 'column', height: '100vh', background: '#f8fafc', color: '#0f172a', fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', minWidth: 0 }}>
          ...
          <div>
            <h2 style={{ fontSize: '1rem', fontWeight: 900, margin: 0, color: '#0f172a', display: 'flex', alignItems: 'center', gap: 6 }}>
              <span>🎓 {submission.studentName || 'Öğrenci'}:</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <div style={{
            ...
            %{scorePercentage} Puan ({totalEarned}/{maxPossible})
          </div>
      ...
      <QuizPanelLayout
        ...
        defaultSize={450}
        documentContent={
          <div style={{ flex: 1, minWidth: 0, height: '100%', background: '#ffffff', color: '#0f172a' }}>
            {isPdf ? (
              ...
              <div style={{ padding: '2rem', maxWidth: 700, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                {questions.map((q, idx) => (
                  <div key={idx} style={{ background: '#ffffff', borderRadius: '1rem', padding: '1.25rem', border: '1.5px solid #e2e8f0', boxShadow: '0 2px 8px rgba(0,0,0,0.03)' }}>
                    <div style={{ fontWeight: 900, fontSize: '0.9rem', color: '#3b82f6', marginBottom: 6 }}>Soru {idx + 1}</div>
                    <div style={{ fontSize: '0.95rem', fontWeight: 600, color: '#0f172a', lineHeight: 1.6 }}>{q.questionText}</div>
                    {q.imageUrl && (
                      <img
                        src={q.imageUrl}
                        alt="Soru Görseli"
                        style={{ maxWidth: '100%', maxHeight: 280, objectFit: 'contain', borderRadius: 8, marginTop: 8, cursor: 'zoom-in' }}
        ...
        answerContent={
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', paddingBottom: '2rem' }}>
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <span style={{
                      fontWeight: 900,
                      fontSize: '0.82rem',
                      color: currentScore === 10 ? '#15803d' : (currentScore >= 5 ? '#d97706' : '#b91c1c')
                    }}>
                  ...
                  {qObj.imageUrl && !isPdf && !isHtml && (
                    <img
                      src={qObj.imageUrl}
                      alt={`Soru ${qNo}`}
                      style={{ maxWidth: '100%', maxHeight: 160, objectFit: 'contain', borderRadius: 6, cursor: 'zoom-in' }}
                      onClick={() => setLightboxSrc(qObj.imageUrl)}
                    />
                  )}
            ...
            <div style={{ background: '#ffffff', padding: '1rem', borderRadius: '0.85rem', border: '1.5px solid #cbd5e1', display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
              <div style={{ fontSize: '0.8rem', fontWeight: 900, color: '#4f46e5' }}>
                💬 Genel Değerlendirme & Karne Notu:
              </div>
              <textarea
                rows="2"
                ...
                style={{
                  ...
                  border: '1px solid #cbd5e1', fontSize: '0.82rem', outline: 'none', boxSizing: 'border-box', resize: 'none'
                }}
              />
              <button
                type="button"
...
export default function EvaluationManager() {
  ...
  const filteredSubmissions = useMemo(() => {
    return enrichedSubmissions.filter(sub => {
      if (activeTab === 'pending' && !sub.isPending) return false;
      if (activeTab === 'completed' && sub.isPending) return false;
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8fafc', color: '#0f172a', padding: '1.5rem 2rem 5rem 2rem', boxSizing: 'border-box', fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      {/* Header */}
      <div style={{ background: '#ffffff', border: '1.5px solid #e2e8f0', borderRadius: '1rem', padding: '1.25rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem', marginBottom: '1.5rem', boxShadow: '0 2px 8px rgba(0,0,0,0.02)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <button
            onClick={() => {
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          <button
            ...
            style={{
              padding: '0.45rem 0.95rem', borderRadius: '0.5rem',
              border: '1px solid #cbd5e1',
              background: activeTab === 'pending' ? '#d97706' : '#f8fafc',
          ...
          <button
            ...
            style={{
              padding: '0.45rem 0.95rem', borderRadius: '0.5rem',
              border: '1px solid #cbd5e1',
              background: activeTab === 'completed' ? '#059669' : '#f8fafc',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1rem' }}>
          {filteredSubmissions.map(sub => (
            ...
            >
              ...
              <div style={{ fontWeight: 700, fontSize: '0.88rem', color: '#334155', lineHeight: 1.4 }}>
                {sub.testTitle}
              </div>
