...
import { 
  ...
  Sparkles, Flame, Clock, Calendar, ShieldCheck, LogIn, AlertCircle, FileText, PlayCircle, HelpCircle, Lock
} from 'lucide-react';
...
export default function Landing() {
  ...
  // Compute live system stats ONLY if user is logged in
  const totalHomeworks = currentUser ? (homeworks?.length || 0) : 0;
  const totalSubmissions = currentUser ? (submissions?.length || 0) : 0;
  const totalPlans = currentUser ? (studyPlans?.length || 0) : 0;
  const totalBooks = currentUser ? (books?.length || 0) : 0;
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: "'Inter', 'Segoe UI', system-ui, sans-serif" }}>
            ...
            <div style={{ animation: 'proFadeUp 0.5s ease both' }}>
              ...
              <div style={{ display: 'flex', gap: '0.85rem', flexWrap: 'wrap' }}>
                {currentUser?.role === 'student' ? (
                  ...
                  <button onClick={() => navigate('/login')} className="pro-btn" style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: 'white', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 16, padding: '0.85rem 1.8rem', fontWeight: 900, fontSize: '0.95rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, boxShadow: '0 8px 24px rgba(99,102,241,0.4)' }}>
                    <LogIn size={20} /> Giriş Yaparak Başla <ArrowRight size={18} />
                  </button>
                )}
              </div>
            </div>

            {/* Right Column: Card (Logged In vs Guest) */}
            {currentUser ? (
              /* LOGGED IN: Dynamic System Overview Card */
              <div style={{ background: 'rgba(255,255,255,0.12)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.25)', borderRadius: 24, padding: isMobile ? '1.25rem' : '1.75rem', animation: 'proFadeUp 0.6s ease both' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyBetween: 'space-between', gap: 8, marginBottom: '1.25rem' }}>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.7)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Sistem Durumunuz</div>
              ...
              /* GUEST: Public Features Card (No Private Data) */
              <div style={{ background: 'rgba(255,255,255,0.12)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.25)', borderRadius: 24, padding: isMobile ? '1.25rem' : '1.75rem', animation: 'proFadeUp 0.6s ease both', color: 'white' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: '1rem' }}>
                  <ShieldCheck size={24} color="#34d399" />
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.7)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Güvenli Portala Hoş Geldiniz</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 900, color: 'white' }}>Neden E-Test Pro?</div>
                  </div>
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.08)', padding: '0.65rem 0.85rem', borderRadius: 14 }}>
                    <Target size={18} color="#fbbf24" />
                    <span style={{ fontSize: '0.82rem', fontWeight: 700 }}>20 Günlük Adım Adım Yol Haritaları</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.08)', padding: '0.65rem 0.85rem', borderRadius: 14 }}>
                    <BarChart3 size={18} color="#34d399" />
                    <span style={{ fontSize: '0.82rem', fontWeight: 700 }}>Anında Optik Karne & Soru Analizi</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.08)', padding: '0.65rem 0.85rem', borderRadius: 14 }}>
                    <BookOpen size={18} color="#38bdf8" />
                    <span style={{ fontSize: '0.82rem', fontWeight: 700 }}>Zengin Soru Bankası & Kitap Takibi</span>
                  </div>
                </div>
              </div>
            )}
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: isMobile ? '1.5rem 1rem' : '2rem', marginTop: -20, position: 'relative', zIndex: 10 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)', gap: isMobile ? '0.85rem' : '1.25rem' }}>
          ...
          <div onClick={() => handleProtectedNavigate('/student')} className="pro-tile"
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              {!currentUser ? <Lock size={18} color="rgba(255,255,255,0.7)" /> : <ChevronRight size={20} color="rgba(255,255,255,0.6)" />}
            </div>
            <div>
          ...
          {/* Tile 2: Kitaplarım */}
          <div onClick={() => handleProtectedNavigate('/student/books')} className="pro-tile"
          ...
          <div onClick={() => handleProtectedNavigate('/study-plans')} className="pro-tile"
          ...
          {/* Tile 4: Karne & Analiz */}
          <div onClick={() => handleProtectedNavigate('/student-results')} className="pro-tile"
          ...
          <div onClick={() => handleProtectedNavigate('/wrong-answers')} className="pro-tile"
          ...
          <div onClick={() => handleProtectedNavigate('/teacher')} className="pro-tile"
      ...
      {/* ════════════════ 3. DYNAMIC WIDGETS OR PUBLIC PROMO ════════════════ */}
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: isMobile ? '1rem' : '1.5rem 2rem 4rem' }}>
        ...
        {currentUser ? (
          ...
          /* UNAUTHENTICATED GUEST PROMO SECTION (NO PRIVATE USER DATA SHOWN) */
          <div style={{ background: 'white', borderRadius: 28, padding: isMobile ? '1.5rem' : '2.5rem', border: '1px solid #e2e8f0', boxShadow: '0 8px 30px rgba(0,0,0,0.04)', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.2fr 0.8fr', gap: '2rem', alignItems: 'center' }}>
            <div>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#eef2ff', color: '#4f46e5', fontWeight: 800, fontSize: '0.75rem', padding: '0.35rem 0.85rem', borderRadius: 99, marginBottom: 12 }}>
                <Lock size={14} /> Güvenli Hesabınızla Başlayın
              </div>
              <h2 style={{ fontSize: isMobile ? '1.35rem' : '1.85rem', fontWeight: 900, color: '#0f172a', margin: '0 0 1rem 0', lineHeight: 1.25 }}>
                Kayıtlı Verilerinize ve Eğitim Panelinize Erişmek İçin Giriş Yapın
              </h2>
              <p style={{ fontSize: '0.9rem', color: '#64748b', lineHeight: 1.6, margin: '0 0 1.5rem 0', fontWeight: 500 }}>
                Ödevleriniz, çözdüğünüz testler, 20 günlük yol haritalarınız ve karneleriniz tamamen gizli ve hesabınıza özeldir. Tüm özelliklere erişmek için giriş yapabilir veya yeni üye olabilirsiniz.
              </p>
              
              <button onClick={() => navigate('/login')} className="pro-btn" style={{ background: 'linear-gradient(135deg, #4f46e5, #7c3aed)', color: 'white', border: 'none', borderRadius: 16, padding: '0.85rem 1.8rem', fontWeight: 900, fontSize: '0.95rem', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8, boxShadow: '0 8px 24px rgba(79,70,229,0.3)' }}>
                <LogIn size={18} /> Hesabına Giriş Yap / Kayıt Ol <ArrowRight size={16} />
              </button>
            </div>

            <div style={{ background: '#f8fafc', borderRadius: 20, padding: '1.5rem', border: '1px solid #cbd5e1' }}>
              <h4 style={{ margin: '0 0 1rem 0', fontWeight: 900, fontSize: '1rem', color: '#0f172a' }}>Platform Ayrıcalıkları:</h4>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.85rem', fontWeight: 700, color: '#334155' }}>
                  <CheckCircle2 size={18} color="#22c55e" /> %100 Kişiselleştirilmiş Yol Haritaları
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.85rem', fontWeight: 700, color: '#334155' }}>
                  <CheckCircle2 size={18} color="#22c55e" /> Anında Optik Form & Soru Analiz Karnesi
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.85rem', fontWeight: 700, color: '#334155' }}>
                  <CheckCircle2 size={18} color="#22c55e" /> Yanlış Sorular Akıllı Tekrar Havuzu
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.85rem', fontWeight: 700, color: '#334155' }}>
                  <CheckCircle2 size={18} color="#22c55e" /> Öğretmen ve Koç Takip Portalı
                </div>
              </div>
            </div>
          </div>
