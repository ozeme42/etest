...
export default function MyCoachingPage() {
  ...
  const { generalTrialExams, otherHomeworkSubmissions } = useMemo(() => {
    ...
    const studentNameClean = (targetStudent?.name || personalInfo?.fullName || currentUser?.name || '').trim().toLowerCase();
    const studentEmailClean = (targetStudent?.email || currentUser?.email || '').trim().toLowerCase();
