  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', background: '#0f172a', color: '#f8fafc' }}>
      ...
      <header style={{ 
        ...
        <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0, flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <span style={{ fontSize: '0.62rem', fontWeight: 900, background: '#4f46e5', color: 'white', padding: '0.12rem 0.45rem', borderRadius: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
              FİZİKİ DENEME
            </span>
            <h2 style={{ 
              color: '#f8fafc', 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '0.35rem' : '0.6rem', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          ...
          {/* Optik Göster / Gizle Button (Full screen PDF toggle) */}
          <button
            onClick={() => {
      ...
      {isTeacherReviewing && currentViewingStudent && (
        <div style={{ padding: '0.6rem 1.25rem', background: '#312e81', borderBottom: '1px solid #4338ca', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.8rem', fontWeight: 800, color: '#e0e7ff' }}>
            <span style={{ background: '#4f46e5', padding: '2px 8px', borderRadius: 6, fontSize: '0.7rem' }}>Öğretmen İncelemesi</span>
            <span>{currentViewingStudent.name} isimli öğrencinin optik formunu inceliyorsunuz.</span>
          </div>
          <button onClick={() => navigate(-1)} style={{ background: '#1e1b4b', border: '1px solid #4338ca', color: '#c7d2fe', padding: '0.3rem 0.8rem', borderRadius: 6, fontSize: '0.75rem', fontWeight: 800, cursor: 'pointer' }}>
            ← Geri
          </button>
        </div>
      )}
      ...
      >
        ...
        {/* RIGHT/BOTTOM: Optical Form Area — scrollable */}
        {showOptikForm && (
          <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', minWidth: 0, background: '#0f172a' }}>
            <div style={{ maxWidth: pdfMode === 'hidden' ? 1200 : undefined, width: '100%', margin: pdfMode === 'hidden' ? '0 auto' : undefined, padding: isMobile ? '0.75rem' : '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              {isSubmitted && results && (
                <div style={{ background: 'linear-gradient(135deg, #312e81 0%, #4338ca 50%, #6d28d9 100%)', borderRadius: '1.25rem', padding: '1rem 1.25rem', color: 'white', boxShadow: '0 8px 24px rgba(67,56,202,0.3)', border: '1px solid rgba(255,255,255,0.15)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div>
                        <div style={{ fontSize: '0.68rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.06em', opacity: 0.85 }}>Sonuç Karnesi</div>
                        <div style={{ fontSize: '1.1rem', fontWeight: 900 }}>{homework.title}</div>
                      </div>
                    </div>

                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                      <div style={{ background: 'rgba(255,255,255,0.12)', borderRadius: 10, padding: '0.4rem 0.75rem', textAlign: 'center' }}>
                        <div style={{ fontSize: '1rem', fontWeight: 900, color: '#4ade80' }}>{results.totalCorrect}</div>
                        <div style={{ fontSize: '0.62rem', fontWeight: 700, opacity: 0.8 }}>Doğru</div>
                      </div>
                      <div style={{ background: 'rgba(255,255,255,0.12)', borderRadius: 10, padding: '0.4rem 0.75rem', textAlign: 'center' }}>
                        <div style={{ fontSize: '1rem', fontWeight: 900, color: '#f87171' }}>{results.totalWrong}</div>
                        <div style={{ fontSize: '0.62rem', fontWeight: 700, opacity: 0.8 }}>Yanlış</div>
                      </div>
                      <div style={{ background: 'rgba(255,255,255,0.12)', borderRadius: 10, padding: '0.4rem 0.75rem', textAlign: 'center' }}>
                        <div style={{ fontSize: '1rem', fontWeight: 900, color: '#cbd5e1' }}>{results.totalBlank}</div>
                      ...
                      <div style={{ background: 'rgba(255,255,255,0.2)', border: '1.5px solid rgba(255,255,255,0.3)', borderRadius: 12, padding: '0.4rem 1.1rem', textAlign: 'center' }}>
                        <div style={{ fontSize: '1.4rem', fontWeight: 900, color: '#4ade80', lineHeight: 1 }}>{results.totalNet}</div>
                        <div style={{ fontSize: '0.62rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.04em', marginTop: 2 }}>Toplam Net</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {subjects.map((sub, idx) => {
                  ...
                  return (
                    ...
                    >
                      <span>{sub.name}</span>
                      {isSubmitted && sStat ? (
                        <span style={{ fontSize: '0.65rem', background: 'rgba(255,255,255,0.2)', padding: '1px 6px', borderRadius: 6, fontWeight: 800 }}>
                          {sStat.net} Net
                        </span>
                      ) : (
                        <span style={{ fontSize: '0.65rem', background: isActive ? 'rgba(255,255,255,0.2)' : '#0f172a', padding: '1px 6px', borderRadius: 6, color: isActive ? 'white' : '#94a3b8', fontWeight: 800 }}>
                          {filled}/{sub.count}
                        </span>
                      )}
                    </button>
              ...
              {activeSubject && (
                <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '1.25rem', padding: isMobile ? '1rem' : '1.25rem', boxShadow: '0 8px 30px rgba(0,0,0,0.35)', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                      ...
                      return (
                        <div 
                          ...
                          style={{
                            ...
                            border: isCorrect ? '1.5px solid #10b981' : isWrong ? '1.5px solid #ef4444' : selected ? '1.5px solid #4f46e5' : '1px solid #334155',
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'space-between',
                            gap: '0.5rem'
                              ...
                              return (
                                <button
                                  ...
                                  style={{
                                    ...
                                    background: isSubmitted && isThisOptCorrect ? '#059669' : isSubmitted && isSelected && isWrong ? '#dc2626' : isSelected ? '#4f46e5' : '#1e293b',
                                    color: isSelected || (isSubmitted && isThisOptCorrect) ? 'white' : '#cbd5e1',
                                    transition: 'all 0.12s ease',
                                    boxShadow: isSelected ? '0 2px 8px rgba(79,70,229,0.4)' : 'none'
                                  }}
                                >
                                  {opt}
                                </button>
      ...
      {showFinishModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 999999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0, 0, 0, 0.65)', backdropFilter: 'blur(5px)' }}>
          <div style={{ width: '100%', maxWidth: '420px', textAlign: 'center', padding: '2rem', background: '#1e293b', borderRadius: '1.5rem', border: '1px solid #334155', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)', display: 'flex', flexDirection: 'column', gap: '1rem', color: '#f8fafc', margin: '1rem' }}>
            <div style={{ width: '60px', height: '60px', borderRadius: '50%', background: 'rgba(239, 68, 68, 0.15)', border: '2px solid #ef4444', color: '#f87171', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto' }}>
              <CheckCircle2 size={30} />
            </div>
            <h3 style={{ margin: 0, fontSize: '1.35rem', fontWeight: 900 }}>Sınavı Bitiriyorsunuz</h3>
      ...
      {/* ── MOBILE FLOATING ACTION BUTTON (<FileSpreadsheet />) ── */}
            ...
            <div style={{ padding: '1rem', overflowY: 'auto', flex: 1, background: '#0f172a' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                {activeSubject && Array.from({ length: activeSubject.count }).map((_, qIdx) => {
                  ...
                  return (
                    <div key={qNo} style={{ background: '#1e293b', padding: '0.75rem 0.85rem', borderRadius: '0.75rem', border: selected ? '1.5px solid #10b981' : '1px solid #334155', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.5rem' }}>
