...
export default function StudentDashboard() {
  ...
  const catchUpTasks = useMemo(() => {
            ...
            if (!isSolved) {
              ...
              if (!seen.has(key)) {
                ...
                list.push({
                  ...
                  isCatchUp: true,
                  reason: `Hedef: ${targetDateObj.toLocaleDateString('tr-TR')}`
                });
              }
            }
          }
    ...
    // 1.2) Tüm Kitaplarda Bireysel Tarihi Geçmiş Testler (Direct Book Test Due Dates)
    (books || []).filter(b => b && b.bookType !== 'exam').forEach(b => {
      const allTestsInBook = [];
      (b.subjects || []).forEach(subj => {
        (subj.tests || []).forEach(t => allTestsInBook.push({ ...t, bookId: b.id }));
        (subj.topics || []).forEach(tp => {
          (tp.tests || []).forEach(t => allTestsInBook.push({ ...t, bookId: b.id }));
        });
      });

      allTestsInBook.forEach(t => {
        const tDue = t.dueDate || t.testDueDate || t.due_date || t.date;
        if (!tDue) return;
        const targetDateObj = parseSafeDate(tDue);
        if (targetDateObj && targetDateObj.getTime() < nowTime) {
          const isSolved = isTestSolvedByStudent(t.id, null);
          if (!isSolved) {
            const key = `book_test_direct_${t.id}`;
            const hwKey = `book_test_${t.id}`;
            if (!seen.has(key) && !seen.has(hwKey)) {
              seen.add(key);
              const info = resolveBookTestInfo(t.id, null, b);
              list.push({
                id: key,
    ...
    // 1.3) bookTests içinde tarihi geçmiş testler (Supabase tracked_book_tests)
    (bookTests || []).forEach(bt => {
      if (!bt) return;
      const tDue = bt.dueDate || bt.testDueDate || bt.due_date || bt.date;
      if (!tDue) return;
      const targetDateObj = parseSafeDate(tDue);
      if (targetDateObj && targetDateObj.getTime() < nowTime) {
        const isSolved = isTestSolvedByStudent(bt.id, null);
        if (!isSolved) {
          const key = `book_test_bt_${bt.id}`;
          const hwKey = `book_test_${bt.id}`;
          const directKey = `book_test_direct_${bt.id}`;
          if (!seen.has(key) && !seen.has(hwKey) && !seen.has(directKey)) {
            seen.add(key);
            const bId = String(bt.bookId || bt.book_id || '');
            const currentBook = (books || []).find(b => String(b.id) === bId || (toUUID(b.id) && toUUID(b.id) === toUUID(bId)));
            const info = resolveBookTestInfo(bt.id, null, currentBook);
            list.push({
              ...
              realTestId: bt.id,
              bookTestId: bt.id,
              bookId: bt.bookId || currentBook?.id,
              isAutoHomework: true,
              isBookTask: true,
              taskType: 'kitap',
              categoryType: 'kitap',
