...
const GOAL_TYPE_CONFIG = {
  Soru:   { color: '#f43f5e', bg: 'bg-rose-500',    light: 'bg-rose-50',       text: 'text-rose-600',       border: 'border-rose-200',   icon: Target,        unit: 'soru'  },
  Sayfa:  { color: '#3b82f6', bg: 'bg-blue-500',    light: 'bg-blue-50',       text: 'text-blue-600',       border: 'border-blue-200',   icon: BookOpen,      unit: 'sayfa' },
  Konu:   { color: '#a855f7', bg: 'bg-purple-500',  light: 'bg-purple-50',   text: 'text-purple-600',   border: 'border-purple-200', icon: Brain,         unit: 'konu'  },
  Dakika: { color: '#10b981', bg: 'bg-emerald-500', light: 'bg-emerald-50', text: 'text-emerald-600', border: 'border-emerald-200', icon: Timer,        unit: 'dk'    },
  Net:    { color: '#06b6d4', bg: 'bg-cyan-500',     light: 'bg-cyan-50',       text: 'text-cyan-600',       border: 'border-cyan-200',   icon: TrendingUp,   unit: 'net'   },
  Puan:   { color: '#f59e0b', bg: 'bg-amber-500',    light: 'bg-amber-50',     text: 'text-amber-600',     border: 'border-amber-200',   icon: Trophy,        unit: 'puan'  },
};

const PERIOD_CONFIG = {
  'Günlük':     { icon: Flame,        badge: 'bg-amber-100 text-amber-700' },
  'Haftalık':   { icon: CalendarClock,badge: 'bg-purple-100 text-purple-700' },
  'Aylık':      { icon: Trophy,       badge: 'bg-indigo-100 text-indigo-700' },
  'Uzun Vadeli':{ icon: GraduationCap,badge: 'bg-emerald-100 text-emerald-700' },
};
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-4 space-y-3 shadow-sm">
      <button
        ...
        className="w-full flex items-center justify-between border-b border-slate-100 pb-2 text-left cursor-pointer hover:opacity-80 transition-opacity"
      >
        ...
        <>
            ...
            {items.map(item => (
              <div
                ...
                className={cn(
                  ...
                  item.done ? 'bg-emerald-50/70 border-emerald-300' : 'bg-slate-50/60 border-slate-200/80'
                )}
              >
                <div className="flex items-center gap-2.5 min-w-0 flex-1">
                  <button
                    type="button"
                    className={cn('w-5 h-5 rounded-lg border-2 flex items-center justify-center shrink-0 transition-all', item.done ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-slate-300 bg-white')}
                  >
                    {item.done && <Check className="w-3.5 h-3.5" strokeWidth={3} />}
                  </button>
                  <span className={cn('text-xs font-bold transition-all line-clamp-2', item.done ? 'line-through text-emerald-700' : 'text-slate-800')}>
                    {item.text}
                  </span>
                </div>
          ...
          <form onSubmit={handleAdd} className="flex gap-2 pt-1">
            <input
              ...
              className="flex-1 px-3 py-1.5 rounded-xl border border-slate-200 bg-slate-50 text-xs font-semibold text-slate-800 outline-none focus:border-indigo-500"
            />
            <button type="submit" className="px-3 py-1.5 rounded-xl bg-slate-900 text-white text-xs font-black hover:opacity-90 transition-all shrink-0 flex items-center gap-1">
...
function DailyHabitStreakSection({ items, onToggleDay, onAddItem, onDeleteItem }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-rose-200 p-4 space-y-3 shadow-sm">
      ...
      <div className="space-y-3 pt-1">
        {items.length === 0 ? (
          ...
          items.map(item => {
            ...
            return (
              <div key={item.id} className="bg-slate-50/70 border border-slate-200/80 rounded-xl p-3 space-y-2 group hover:border-rose-300 transition-all">
                ...
                <div className="grid grid-cols-7 gap-1">
                  {WEEK_DAYS.map(day => {
                    ...
                    return (
                      ...
                      >
                        <span>{day}</span>
                        <div className={cn('w-2.5 h-2.5 rounded-full mt-1 flex items-center justify-center', done ? 'bg-white' : 'bg-slate-200')}>
      ...
      <form onSubmit={handleAdd} className="flex gap-2 pt-1">
        <input
          ...
          value={newText}
          onChange={e => setNewText(e.target.value)}
          className="flex-1 px-3 py-1.5 rounded-xl border border-slate-200 bg-slate-50 text-xs font-semibold text-slate-800 outline-none focus:border-rose-500"
        />
...
function WeeklyHabitStreakSection({ items, onToggleWeek, onAddItem, onDeleteItem }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-purple-200 p-4 space-y-3 shadow-sm">
      <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
      ...
      <div className="space-y-3 pt-1">
        {items.length === 0 ? (
          ...
          items.map(item => {
            ...
            return (
              <div key={item.id} className="bg-slate-50/70 border border-slate-200/80 rounded-xl p-3 space-y-2 group hover:border-purple-300 transition-all">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
      ...
      <form onSubmit={handleAdd} className="flex gap-2 pt-1">
        <input
          ...
          value={newText}
          onChange={e => setNewText(e.target.value)}
          className="flex-1 px-3 py-1.5 rounded-xl border border-slate-200 bg-slate-50 text-xs font-semibold text-slate-800 outline-none focus:border-purple-500"
        />
...
function VisualGoalCard({ goal, onDelete, onAddProgress }) {
  ...
  return (
    <div className={cn(
      'relative bg-white rounded-2xl border shadow-sm flex flex-col gap-3 p-4 group transition-all hover:shadow-md',
      done ? 'border-emerald-400/60 ring-1 ring-emerald-400/20' : 'border-slate-100'
    )}>
      {done && (
      ...
      <button onClick={() => onDelete(goal.id)} className="absolute top-3 right-3 p-1 text-slate-300 hover:text-rose-500 rounded-lg transition-all opacity-0 group-hover:opacity-100">
        <Trash2 className="w-3.5 h-3.5" />
      </button>
      ...
      <div className="flex items-start gap-3.5">
        <div className="relative shrink-0">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className={cn('text-xs font-black', done ? 'text-emerald-500' : 'text-slate-800')}>{pct}%</span>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0 pt-0.5">
          ...
          <h3 className="text-sm font-bold text-slate-800 leading-snug line-clamp-2">{goal.title}</h3>
        </div>
      </div>

      <div className="space-y-1.5 pt-1">
        <div className="flex justify-between text-[11px] font-black text-slate-500">
          <span>İlerleme: <strong className="text-slate-800">{goal.current || 0}</strong> / {goal.target} {t.unit}</span>
        ...
        <div className="flex items-center gap-2 pt-1">
          ...
          <form onSubmit={handleAdd} className="flex-1 flex gap-1.5">
            <input
              ...
              className="flex-1 px-3 py-1 rounded-xl border border-slate-200 bg-slate-50 text-xs font-semibold text-slate-800 outline-none focus:border-indigo-500"
            />
            <button
              type="submit"
              className="px-3 py-1 rounded-xl bg-slate-900 text-white text-xs font-black hover:opacity-90 transition-all shrink-0"
            >
              Ekle
            </button>
          </form>
...
export default function GoalsAndSchedulePage() {
  ...
  return (
    <div className="min-h-screen text-slate-800 p-4 sm:p-6 pb-20" style={{ background: 'linear-gradient(160deg,#f0f4ff 0%,#ffffff 55%,#fdf4ff 100%)', fontFamily: 'inherit' }}>
          ...
          <div className="flex items-center gap-2 bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm">
            {students.map(s => (
              <button key={s.id} onClick={() => setSelectedStudentId(s.id)} className={cn(
                ...
                s.id === selectedStudent?.id ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800:text-white'
              )}>
                {s.name}
              </button>
            ))}
      ...
      <div className="max-w-7xl mx-auto mb-6 bg-gradient-to-r from-emerald-500/10 via-teal-500/10 to-indigo-500/10 border border-emerald-500/30 rounded-2xl p-4 flex items-center gap-3.5 shadow-sm">
        ...
        <div>
          <h4 className="text-sm font-black text-emerald-600">🔄 1-e-1 Birebir Alışkanlık & Seri Takip Senkronizasyonu</h4>
      ...
      <div className="max-w-7xl mx-auto mb-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-white border border-rose-200 rounded-2xl p-3.5 flex items-center gap-3 shadow-sm">
          ...
          <div>
            ...
            <span className="text-base font-black text-slate-900">{totalQuestionSolved} Soru</span>
          </div>
        </div>

        <div className="bg-white border border-blue-200 rounded-2xl p-3.5 flex items-center gap-3 shadow-sm">
          ...
          <div>
            ...
            <span className="text-base font-black text-slate-900">{totalPagesRead} Sayfa</span>
          </div>
        </div>

        <div className="bg-white border border-purple-200 rounded-2xl p-3.5 flex items-center gap-3 shadow-sm">
          ...
          <div>
            ...
            <span className="text-base font-black text-slate-900">{totalTopicsCompleted} Konu</span>
          </div>
        </div>

        <div className="bg-white border border-emerald-200 rounded-2xl p-3.5 flex items-center gap-3 shadow-sm">
          ...
          <div>
            ...
            <span className="text-base font-black text-slate-900">{totalMinutesStudied} Dakika</span>
          </div>
        </div>
      </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-slate-50 text-xs font-bold text-slate-800 outline-none"
                    >
                  ...
                  {(!isStandardExam || examGoalType === 'Özel Sınav') && (
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 rounded-xl border border-purple-300 bg-purple-50/50 text-xs font-bold text-slate-800 outline-none"
                      />
                    </div>
                  )}
                    ...
                    <>
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className="w-full px-3 py-2 rounded-xl border border-emerald-300 bg-emerald-50/50 text-xs font-bold text-slate-800 outline-none"
                        >
        ...
        <div className="lg:col-span-6 space-y-4">
              ...
              {['Tümü', 'Günlük', 'Haftalık', 'Aylık'].map(p => (
                <button
                  ...
                  className={cn(
                    ...
                    periodFilter === p ? 'bg-indigo-600 text-white shadow-xs' : 'text-slate-500 hover:text-slate-800:text-white'
                  )}
                >
                  {p}
                </button>
            ...
            <div className="flex items-center gap-1 bg-white p-1 rounded-xl border border-slate-200 shadow-sm shrink-0">
              {['Tümü', 'Soru', 'Sayfa', 'Konu', 'Dakika'].map(t => (
                <button
                  ...
                  className={cn(
                    'px-2.5 py-1 rounded-lg text-xs font-bold transition-all',
                    typeFilter === t ? 'bg-rose-500 text-white shadow-xs' : 'text-slate-500 hover:text-slate-800:text-white'
                  )}
                >
                  {t}
                </button>
          ...
          {filteredVisualGoals.length === 0 ? (
            <div className="bg-white border-2 border-dashed border-slate-200 rounded-2xl p-10 text-center space-y-3">
              ...
              <h3 className="text-sm font-bold text-slate-800">Seçilen Kriterlere Uygun Özel Hedef Bulunmuyor</h3>
          ...
          <div className="bg-white rounded-3xl p-6 w-full max-w-md border border-slate-200 shadow-2xl space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-black text-slate-900 text-base">Yeni Özel Hedef Tanımla</h3>
              </div>
              <button onClick={() => setShowGoalModal(false)} className="p-1 rounded-lg text-slate-400 hover:text-slate-600:text-white">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveGoal} className="space-y-3">
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1">Hedef Tanımı / Başlığı</label>
                <input
                  type="text"
                  placeholder="Örn: Günlük 30 Paragraf Sorusu / 50 Sayfa Kitap"
                  value={newGoal.title}
                  onChange={e => setNewGoal(p => ({ ...p, title: e.target.value }))}
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm font-semibold text-slate-800 outline-none focus:border-rose-500"
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm font-semibold text-slate-800 outline-none"
                  >
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">Periyot</label>
                  <select
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1">Hedef Miktar</label>
                <input
                  type="number"
                  min="1"
                  placeholder="Örn: 50"
              ...
              <div className="pt-2 flex justify-end gap-2">
                <button type="button" onClick={() => setShowGoalModal(false)} className="px-4 py-2 rounded-xl text-xs font-bold text-slate-500 hover:bg-slate-100:bg-slate-800">İptal</button>
