...
export default function MultipleChoiceReview({
  ...
  const [mistakeReason, setMistakeReason] = useState(() => {
    ...
    if (hasValidQId) {
      try {
        return localStorage.getItem(`mistake_mc_${question.id}`) || '';
      } catch {}
    }
    return '';
  });
  ...
  useEffect(() => {
    ...
    if (hasValidQId) {
      try {
        const saved = localStorage.getItem(`mistake_mc_${question.id}`);
        if (saved) {
  ...
  // Clean up legacy generic keys like mistake_mc_1, mistake_mc_2 if found
  useEffect(() => {
    try {
      if (typeof window !== 'undefined' && window.localStorage) {
        for (let i = localStorage.length - 1; i >= 0; i--) {
          const k = localStorage.key(i);
          if (k && /^mistake_mc_\d+$/.test(k)) {
  ...
  const handleSetMistakeReason = (reason) => {
    ...
    if (hasValidQId) {
      try {
        if (next) {
          localStorage.setItem(`mistake_mc_${question.id}`, next);
        } else {
          localStorage.removeItem(`mistake_mc_${question.id}`);
        }
      } catch {}
    }
  };
    ...
    >
      ...
      {aiModalOpen && (
        <ScreenSnipperAndSolverModal
          ...
          question={{
            ...question,
            questionText: qText || question?.questionText || '',
            options: rawOptions.length > 0 ? rawOptions : (question?.options || [])
          }}
