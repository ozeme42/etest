    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <div style={{
        ...
        <div>
          <h3 style={{ margin: 0, fontWeight: 900, fontSize: '1.1rem', color: 'var(--color-text)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            Sınıfım &amp; Öğrenci Başarı Analiz Listesi
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', width: '100%' }}>
          ...
          <input
            type="text"
            placeholder="Öğrenci adı veya e-posta ile filtreleyin..."
            value={searchQuery}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
          {filteredStudents.map((student, i) => {
            ...
            return (
              ...
              >
                {/* ── TOP ROW: STUDENT AVATAR, NAME, CLASS SELECTOR, PASSWORD & MAIN ACTIONS ── */}
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem' }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                        <h4
                          ...
                          style={{ margin: 0, fontWeight: 900, fontSize: '0.98rem', color: 'var(--color-text)', cursor: 'pointer' }}
                  ...
                  {/* Right: Class Selector & Action Buttons */}
                ...
                {/* ── SUBJECT BREAKDOWN CHIPS (DERS DERS DAĞILIM) ── */}
                ...
                {/* ── EXPAND/COLLAPSE TOPIC DRILLDOWN BUTTON ── */}
                <div style={{ display: 'flex', justifyContent: 'flex-end', borderTop: '1px solid var(--color-border)', paddingTop: '0.5rem' }}>
                  <button
                    type="button"
                  ...
                  >
                    <span>{isExpanded ? 'Konu Dökümünü Gizle' : `📖 Detaylı Konu Konu Başarı Dökümü (${st.topics.length} Konu)`}</span>
                    {isExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                  </button>
                </div>

                {/* ── 5. EXPANDABLE TOPIC DRILLDOWN ACCORDION ── */}
                {isExpanded && (
                  <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <h5 style={{ margin: 0, fontWeight: 900, fontSize: '0.85rem', color: 'var(--color-text)', display: 'flex', alignItems: 'center', gap: 5 }}>
                        <Layers size={15} color="#6366f1" />
                        {student.name} — Konu Bazlı Çözülen Soru ve Başarı Dökümü
                      </h5>
                      <span style={{ fontSize: '0.72rem', color: 'var(--color-text-muted)', fontWeight: 700 }}>
                        {st.topics.length} farklı konu çalışıldı
                      </span>
                    </div>

                    {st.topics.length === 0 ? (
                      <div style={{ textAlign: 'center', padding: '1rem 0', color: 'var(--color-text-muted)', fontSize: '0.78rem' }}>
                        Bu zaman aralığında çözülen konu verisi bulunmuyor.
                      </div>
                    ) : (
