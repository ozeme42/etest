...
if (!code.includes('DENEME EKLEME MODAL POPUP')) {
  const modalCode = `
      ...
      {showMockModal && (() => {
        const inp = { width: '100%', padding: '0.65rem 0.85rem', borderRadius: '0.75rem', border: '1.5px solid #e2e8f0', fontSize: '0.85rem', fontWeight: 700, color: '#334155', background: 'rgba(255, 255, 255, 0.8)', outline: 'none', boxSizing: 'border-box' };
        const lbl = { display: 'block', fontSize: '0.78rem', fontWeight: 800, color: '#475569', marginBottom: '0.4rem', marginLeft: '0.2rem' };
        return (
...
`;
  ...
  code = code.replace(/    <\/div>\s*<\/div>\s*\);\s*\}\s*$/m, `    </div>
${modalCode}`);
