...
// 2. Fix the useCoaching hooks (removing getMockExamsForStudent and duplicate isSaving)
...
const hookReplacement = `  const { mockExams = [], addMockExam } = useCoaching();
...
// Fix dependencies of allExamsList
code = code.replace(`mockExams.forEach(mock => {`, `studentMockExams.forEach(mock => {`);
code = code.replace(`mock.totalEmpty,`, `mock.totalBlank || mock.totalEmpty,`);
code = code.replace(`mock.totalNet,`, `mock.netScore || mock.totalNet,`);
...
// 3. Exact String Injection for Button
...
if (buttonPos > -1) {
  const insertStr = `<div style={{ display: 'flex', gap: '0.5rem' }}>\n            <button className="btn btn-outline" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', borderColor: '#8b5cf6', color: '#8b5cf6', background: 'white', padding: '0.75rem 1.25rem', borderRadius: '0.75rem', fontWeight: 800, cursor: 'pointer' }} onClick={() => setShowMockModal(true)}>\n              <ClipboardList size={18} /> Manuel Sonuç Ekle\n            </button>\n            `;
  code = code.substring(0, buttonPos) + insertStr + code.substring(buttonPos);
  const endButtonPos = code.indexOf(`Kendi Denemeni Ekle\n          </button>`, buttonPos);
  code = code.substring(0, endButtonPos + 40) + `\n          </div>` + code.substring(endButtonPos + 40);
  console.log('Button injected');
}

// 4. Exact String Injection for Cards
const cardsPosStr = `onClick={(e) => { e.stopPropagation(); navigate(\`/student/books/\${book.id}\`); }}\n                >\n                  {isCompleted ? 'Haritayı Görüntüle' : 'Denemeye Devam Et'} <ArrowRight size={18} />\n                </button>\n              </div>\n            );\n          })}\n        </div>`;
const cardsPos = code.indexOf(cardsPosStr);
if (cardsPos > -1) {
  const mockCardsHTML = `
  ...
  // Replace the closing div with the loop + closing div
  code = code.replace(cardsPosStr, cardsPosStr.replace('        </div>', mockCardsHTML));
  console.log('Cards injected');
}

// 5. Exact String Injection for Modal
const modalPosStr = `      {/* NEW BOOK MODAL */}`;
const modalPos = code.indexOf(modalPosStr);
if (modalPos > -1) {
  const modalHTML = `
      {/* MANUAL MOCK MODAL */}
      {showMockModal && (
  ...
  code = code.replace(modalPosStr, modalHTML);
