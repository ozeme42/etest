...
// 2. Logic (Remove old getMockExamsForStudent hook and insert useCoaching and modal state)
const oldHookRegex = /\\s*const \\[mockExams, setMockExams\\] = useState\\(\\[\\]\\);[\\s\\S]*?\\}, \\[studentId, getMockExamsForStudent\\]\\);/;
const logicInsert = \`
...
code = code.replace(oldHookRegex, logicInsert);

// Also remove duplicate isSaving
code = code.replace(/\\s*const \\[isSaving, setIsSaving\\] = useState\\(false\\);\\n/, '\\n');

// 3. Fix allExamsList dependencies
code = code.replace(/mockExams\.forEach\\(mock => \\{/g, 'studentMockExams.forEach(mock => {');
code = code.replace(/totalEmpty: mock\\.totalEmpty,/g, 'totalEmpty: mock.totalBlank || mock.totalEmpty,');
code = code.replace(/totalNet: mock\\.totalNet,/g, 'totalNet: mock.netScore || mock.totalNet,');
code = code.replace(/scores: mock\\.scores \\|\\| \\{\\}/g, 'scores: mock.subjects || mock.scores || {}');
code = code.replace(/\\}, \\[mockExams, assignedBooks, studentSubmissions\\]\\);/g, '}, [studentMockExams, assignedBooks, studentSubmissions]);');


// 4. Inject Button
const btnRegex = /<button[\\s\\S]*?<Plus size=\\{20\\} \\/> Kendi Denemeni Ekle[\\s\\S]*?<\\/button>/;
const newBtnHtml = \`<div style={{ display: 'flex', gap: '0.5rem' }}>
...
code = code.replace(btnRegex, newBtnHtml);


// 5. Inject Cards
const cardsRegex = /<ArrowRight size=\\{18\\} \\/>[\\s\\S]*?<\\/button>[\\s\\S]*?<\\/div>[\\s\\S]*?\\);[\\s\\S]*?\\}\\)}\\s*<\\/div>/;
const match = code.match(cardsRegex);
if (match) {
  const cardsHtml = match[0] + \`
  ...
  code = code.replace(cardsRegex, cardsHtml);
}


// 6. Inject Modal
const modalRegex = /\\{\\/\\* NEW BOOK MODAL \\*\\/\\}/;
const modalHtml = \`{/* MANUAL MOCK MODAL */}
      ...
      {/* NEW BOOK MODAL */}\`;
code = code.replace(modalRegex, modalHtml);

fs.writeFileSync('src/pages/StudentExamsPage.jsx', code);
console.log('All changes applied cleanly via Regexes.');
